...
void FPDOptionalPinManager::CustomizePinData(UEdGraphPin* Pin, FName SourcePropertyName, int32 ArrayIndex, FProperty& Property, UPDMissionGraphNode* OwnerNode, EPDPinCustomizer CurrentPinHandling) const
{
		...
		(bSkipCurrentPin || (bSkipCurrentPin == false && Schema->IsPinDefaultValid(Pin, NewDefaultValue, nullptr, FText::GetEmpty()).IsEmpty())))
	{
		...
		{
			bool bIsCurrentPinLoopForExistingMission = false;
			const void* ValuePtr = GetPropertyValuePtr(&Property, SampleStructMemory);
			if (const FGameplayTag* TagPtr = reinterpret_cast<const FGameplayTag*>(ValuePtr))
			{
				TArray<FGameplayTagContainer> TagContainers;
				TagContainers.Add(FGameplayTagContainer(*TagPtr));

				// @todo actually bind this widget somehow so we have a selctor dropdown..
				TSharedRef<SWidget> CustomTagWidget = SNew(SGameplayTagCombo)
					.Visibility(EVisibility::Visible)
					.Filter(Property.GetMetaData(TEXT("Categories")))
					// .Tag(this, &SGameplayTagGraphPin::GetGameplayTag)
					.OnTagChanged_Lambda(
						[Pin](const FGameplayTag NewTag)
						{
							Pin->GetSchema()->TrySetDefaultValue(*Pin, NewTag.ToString());
							Pin->GetOwningNode()->Modify();
						});

				// Skipping this, I did not think through this clearly. We don't want to start newly created node wiht a hidden tag
				// @todo revist in when generating graphs and we call CreateVisiblePins on anyhting that ins't an entry node 
				{
					// // if (TagPtr->ToString() != FPDMissionUtility::NewMissionRowLabel.ToString() && SourcePropertyName.ToString() == FString("MissionBaseTag"))
					// if (TagPtr->ToString() != TAG_MakeNewMission.GetTag().ToString() && SourcePropertyName.ToString() == FString("MissionBaseTag"))
					// {
					// 	bIsCurrentPinLoopForExistingMission = true;
					// }
				}
			}
			...
			// @todo revist in when generating graphs and we call CreateVisiblePins on anyhting that ins't an entry node 
			{
				// Hide gameplay mission base tag. 
				// todo: Need to keep it visible for any labels with -- New Mission --. 
				// todo2: need to ensure I have an empty base tag I can assign to the New Mission tag, so I can make proper comparisons of tags instead of this rickety nonsense
				// if (bIsCurrentPinLoopForExistingMission)
				// {
				// 	Pin->SafeSetHidden(true);
				// }
				// else
				// {
				//	Schema->SetPinAutogeneratedDefaultValueBasedOnType(Pin); //, NewDefaultValue);
				// }
			}
