...
public:
	...
	typedef struct RowOp
	{
		...
		static PDMISSIONCORE_API bool IsMissionValid(const FName& SelectedMissionRowName, FDataTableRowHandle*& OutRowHandlePtr);
		static PDMISSIONCORE_API void SetValuesOnBranchTargetAtIndex(const FName& SourceMissionRowName, int32 BranchIdx, const FName& BranchTargetMissionName, const FString Ctxt = TEXT(""));

	private:
	};
private:
	static RowOp RowOpInst;
};
